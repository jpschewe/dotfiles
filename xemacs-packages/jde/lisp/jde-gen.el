...
(defun jde-gen-final-method-modifier (modifiers)
  "Inserts the final modifier depending on `jde-gen-final-methods'."
  (let ((class-info (jde-parse-get-class-modifiers)))
    ...
    (unless (or (member "final" class-info) ; explicit final modifier?
      ...
      (if (and jde-gen-final-methods
               (not (string-match "final" modifiers))
               (not (string-match "private" modifiers))
               (not (string-match "abstract" modifiers)))
          ;; Find correct position according to
          ;; Java Language specification, section 8.4.3.
          ;; http://java.sun.com/docs/books/jls/second_edition/html/classes.doc.html
          (let* ((pos (max (if (string-match "public" modifiers) (match-end 0) 0)
                           (if (string-match "protected" modifiers) (match-end 0) 0)
                           (if (string-match "static" modifiers) (match-end 0) 0)))
                 (left (substring modifiers 0 pos))
                 (right (substring modifiers pos)))
            (setq modifiers (concat
                             left
                             (if (> (length left) 0) " ")
                             "final"
...
(defmacro jde-gen-save-excursion (&rest body)
  "Evaluates BODY within a let form.
...
(defun jde-gen-electric-brace (&optional padding)
  "Inserts a brace according to `jde-gen-k&r'.
...
(defun jde-gen-indent ()
  "Indents current line if it is not empty.
...
(let ((sig
       (concat
        ...
        "("
        (if (> (length arglist) 0)
            (concat jde-gen-method-signature-padding-2 (jde-gen-final-argument-modifier arglist) ; MS
...
(defcustom jde-gen-deep-clone-template
  '(
    ...
    "\"* Create a deep clone of the object.\" '>'n"
    "'> (progn (jde-javadoc-insert-end-block) nil)"

    ;; create method declaration
    "\" public Object clone()\""
    "(unless jde-gen-k&r '(l > n)) \" {\" '>'n"
    ...
    "(progn (goto-char (scan-lists (point) 1 1)) (end-of-line) '(l n n))"
