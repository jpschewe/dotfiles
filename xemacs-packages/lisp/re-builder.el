...
;; Copyright (C) 1999, 2000 by Detlev Zundel
...
;; according to their name.  The other two allow editing of symbolic
;; regular expressions supported by the packages of the same name.
;; (`lisp-re' is a package by me and its support may go away as it is
;; nearly the same as the `sregex' package in Emacs)
...
;; modi.  For the `sregex' syntax the function `sregex' is applied to
;; the evaluated expression read.  So you can use quoted arguments
;; with something like '("text") or you can construct arguments to
...
;; The input syntax can be changed transparently for the obvious
;; exception non-symbolic -> symbolic so you can change your mind as
;; often as you like.
...
(if (not reb-subexp-mode-map)
    (progn
      ...
      ;; Again copy the "\C-c" keymap for the commands
...
optional fourth argument FORCE is non-nil."
  (let ((prev-valid reb-valid-string)
    ...
    ;; Through the caching of the re a change to invalid syntax for
    ;; symbolic expressions will not delete the overlays so we catch
    ;; it here
...
(defun reb-delete-overlays ()
  "Delete all RE Builder overlays in the `reb-target-window' window."
  (with-current-buffer reb-target-buffer
