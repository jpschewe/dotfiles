  ...
  (let ((ask-again t))
    (while ask-again
      (let* ((cursor-in-echo-area t) ; Ensures that the cursor is in 
            ...
            (key (pm-read-key-sequence "Bind macro to which key? (RET to quit) ")))
        (if (member key '("\C-m" [return] "\C-g" [(control ?g)]))
            (progn
              (message 
